...
__global__ void t8gpu::kepes_compute_fluxes(
    MeshConnectivityAccessor<typename variable_traits<VariableList>::float_type, CompressibleEulerSolver::dim>
                                    connectivity,
  ...
  int const num_local_faces = connectivity.get_num_local_faces();
  ...
  float_type F[5];
  for (size_t k = 0; k < 5; k++) F[k] = F_star[k] - nc::half * diss_[k];
...
__global__ void t8gpu::reflective_boundary_condition(
    MeshConnectivityAccessor<typename variable_traits<VariableList>::float_type, CompressibleEulerSolver::dim>
                                    connectivity,
    ...
    typename variable_traits<VariableList>::float_type* __restrict__ speed_estimates) {
  int const i               = blockIdx.x * blockDim.x + threadIdx.x;
  int const num_local_faces = connectivity.get_num_local_boundary_faces();
  if (i >= num_local_faces) return;
...
__global__ void t8gpu::estimate_gradient(MeshConnectivityAccessor<typename variable_traits<VariableList>::float_type,
                                                                  CompressibleEulerSolver::dim> connectivity,
