...
t8gpu::MeshManager<VariableType, StepType, dim>::MeshManager(sc_MPI_Comm      comm,
    ...
    : m_comm{comm},
      ...
      m_cmesh{cmesh},
      m_forest{forest},
      t8gpu::MemoryManager<VariableType, StepType>{static_cast<size_t>(t8_forest_get_local_num_elements(forest)),
                                                   comm} {
...
void t8gpu::MeshManager<VariableType, StepType, dim>::initialize_variables(Func func) {
  ...
  T8GPU_CUDA_CHECK_ERROR(cudaMemset(
      device_element_data_ptr, 0, (nb_variables * nb_steps + 1) * sizeof(float_type) * m_num_local_elements));
...
template<typename VariableType, typename StepType, size_t dim>
void t8gpu::MeshManager<VariableType, StepType, dim>::adapt(
    thrust::host_vector<typename t8gpu::MeshManager<VariableType, StepType, dim>::float_type> const&
                                                                              refinement_criteria,
    typename t8gpu::MeshManager<VariableType, StepType, dim>::step_index_type step) {
...
template<typename VariableType, typename StepType, size_t dim>
t8gpu::MeshConnectivityAccessor<typename t8gpu::MeshManager<VariableType, StepType, dim>::float_type, dim>
t8gpu::MeshManager<VariableType, StepType, dim>::get_connectivity_information() const {
...
template<typename VariableType, typename StepType, size_t dim>
void t8gpu::MeshManager<VariableType, StepType, dim>::save_variable_to_vtk(
    t8gpu::MeshManager<VariableType, StepType, dim>::step_index_type     step,
    t8gpu::MeshManager<VariableType, StepType, dim>::variable_index_type variable,
    std::string const&                                                   prefix) const {
  thrust::host_vector<float_type> element_variable(m_num_local_elements);
  T8GPU_CUDA_CHECK_ERROR(cudaMemcpy(element_variable.data(),
...
template<typename VariableType, typename StepType, size_t dim>
[[nodiscard]] t8gpu::MeshManager<VariableType, StepType, dim>::HostVariableInfo
t8gpu::MeshManager<VariableType, StepType, dim>::get_host_scalar_variable(
    t8gpu::MeshManager<VariableType, StepType, dim>::step_index_type     step,
    t8gpu::MeshManager<VariableType, StepType, dim>::variable_index_type variable,
...
template<typename VariableType, typename StepType, size_t dim>
[[nodiscard]] t8gpu::MeshManager<VariableType, StepType, dim>::HostVariableInfo
t8gpu::MeshManager<VariableType, StepType, dim>::get_host_vector_variable(
    t8gpu::MeshManager<VariableType, StepType, dim>::step_index_type                    step,
    std::array<t8gpu::MeshManager<VariableType, StepType, dim>::variable_index_type, 3> variables,
    std::string const&                                                                  name) const {
...
template<typename VariableType, typename StepType, size_t dim>
void t8gpu::MeshManager<VariableType, StepType, dim>::save_variables_to_vtk(
    std::vector<t8gpu::MeshManager<VariableType, StepType, dim>::HostVariableInfo> host_variables,
    std::string const&                                                             prefix) const {
...
void t8gpu::MeshManager<VariableType, StepType, dim>::partition(step_index_type step) {
  ...
  for (size_t l = 0; l < t8gpu::variable_traits<VariableType>::nb_variables; l++) {
    new_variables[l] =
        thrust::raw_pointer_cast(device_new_conserved_variables.data()) + l * num_new_elements;
  }
