...
inline t8gpu::SharedDeviceVector<T>::SharedDeviceVector(size_t size, sc_MPI_Comm comm)
    : m_size{size}, m_comm{comm}, m_capacity{size} {
  ...
  MPI_Allgather(
      MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, m_handles.data(), sizeof(SharedDeviceVector<T>::Handle), MPI_BYTE, m_comm);

  for (int i = 0; i < m_nb_ranks; i++) {
    if (i != m_rank && m_handles[i].need_open_ipc) {
      T8GPU_CUDA_CHECK_ERROR(cudaIpcOpenMemHandle(
          reinterpret_cast<void**>(&m_arrays[i]), m_handles[i].handle, cudaIpcMemLazyEnablePeerAccess));
    }
  }
...
template<typename T>
inline t8gpu::SharedDeviceVector<T> const& t8gpu::SharedDeviceVector<T>::operator=(
    thrust::host_vector<T> const& other) {
  this->resize(other.size());
  T8GPU_CUDA_CHECK_ERROR(
      cudaMemcpy(m_arrays[m_rank], thrust::raw_pointer_cast(other.data()), sizeof(T) * m_size, cudaMemcpyHostToDevice));
  return *this;
}

template<typename T>
inline t8gpu::SharedDeviceVector<T> const& t8gpu::SharedDeviceVector<T>::operator=(
    thrust::device_vector<T> const& other) {
  this->resize(other.size());
  T8GPU_CUDA_CHECK_ERROR(cudaMemcpy(
      m_arrays[m_rank], thrust::raw_pointer_cast(other.data()), sizeof(T) * m_size, cudaMemcpyDeviceToDevice));
  return *this;
}

template<typename T>
[[nodiscard]] inline size_t t8gpu::SharedDeviceVector<T>::size() const {
  return m_size;
}

template<typename T>
inline void t8gpu::SharedDeviceVector<T>::clear() {
  m_size = 0;
}

template<typename T>
[[nodiscard]] inline T* t8gpu::SharedDeviceVector<T>::get_own() {
  return m_arrays[m_rank];
}

template<typename T>
[[nodiscard]] inline T** t8gpu::SharedDeviceVector<T>::get_all() {
  return thrust::raw_pointer_cast(m_device_arrays.data());
}

template<typename T>
[[nodiscard]] inline T const* t8gpu::SharedDeviceVector<T>::get_own() const {
  return m_arrays[m_rank];
}

template<typename T>
[[nodiscard]] inline T const* const* t8gpu::SharedDeviceVector<T>::get_all() const {
  return thrust::raw_pointer_cast(m_device_arrays.data());
}
...
inline t8gpu::SharedDeviceVector<std::array<T, N>>::SharedDeviceVector(size_t size, sc_MPI_Comm comm)
  ...
  MPI_Allgather(MPI_IN_PLACE,
                ...
                MPI_DATATYPE_NULL,
                m_handles.data(),
                sizeof(SharedDeviceVector<std::array<T, N>>::Handle),
                MPI_BYTE,
                m_comm);
  ...
  for (int i = 0; i < m_nb_ranks; i++) {
    if (i != m_rank && m_handles[i].need_open_ipc) {
      T8GPU_CUDA_CHECK_ERROR(cudaIpcOpenMemHandle(
          reinterpret_cast<void**>(&m_allocations[i]), m_handles[i].handle, cudaIpcMemLazyEnablePeerAccess));
    }
  }
...
template<typename T, size_t N>
inline t8gpu::SharedDeviceVector<std::array<T, N>>& t8gpu::SharedDeviceVector<std::array<T, N>>::operator=(
    SharedDeviceVector&& other) {
  this->~SharedDeviceVector();
  m_rank          = other.m_rank;
  m_nb_ranks      = other.m_nb_ranks;
  m_size          = other.m_size;
...
template<typename T, size_t N>
inline void t8gpu::SharedDeviceVector<std::array<T, N>>::copy(size_t index, thrust::host_vector<T> const& vector) {
  assert(m_size <= vector.size());
  T8GPU_CUDA_CHECK_ERROR(cudaMemcpy(m_arrays[index * m_nb_ranks + m_rank],
...
template<typename T, size_t N>
inline void t8gpu::SharedDeviceVector<std::array<T, N>>::copy(size_t index, thrust::device_vector<T> const& vector) {
  assert(m_size <= vector.size());
  T8GPU_CUDA_CHECK_ERROR(cudaMemcpy(m_arrays[index * m_nb_ranks + m_rank],
                                    thrust::raw_pointer_cast(vector.data()),
                                    sizeof(T) * vector.size(),
...
inline void t8gpu::SharedDeviceVector<std::array<T, N>>::copy(size_t index, T const* buffer, size_t num_elements) {
  ...
  T8GPU_CUDA_CHECK_ERROR(
      cudaMemcpy(m_arrays[index * m_nb_ranks + m_rank], buffer, sizeof(T) * num_elements, cudaMemcpyDeviceToDevice));
}

template<typename T, size_t N>
[[nodiscard]] inline size_t t8gpu::SharedDeviceVector<std::array<T, N>>::size() const {
  return m_size;
}

template<typename T, size_t N>
inline void t8gpu::SharedDeviceVector<std::array<T, N>>::clear() {
  m_size = 0;
}

template<typename T, size_t N>
[[nodiscard]] inline T* t8gpu::SharedDeviceVector<std::array<T, N>>::get_own(int index) {
  return m_arrays[index * m_nb_ranks + m_rank];
}

template<typename T, size_t N>
[[nodiscard]] inline T** t8gpu::SharedDeviceVector<std::array<T, N>>::get_all(int index) {
  return thrust::raw_pointer_cast(m_device_arrays.data() + index * m_nb_ranks);
}

template<typename T, size_t N>
[[nodiscard]] inline T const* t8gpu::SharedDeviceVector<std::array<T, N>>::get_own(int index) const {
  return m_arrays[index * m_nb_ranks + m_rank];
}

template<typename T, size_t N>
[[nodiscard]] inline T const* const* t8gpu::SharedDeviceVector<std::array<T, N>>::get_all(int index) const {
  return thrust::raw_pointer_cast(m_device_arrays.data() + index * m_nb_ranks);
}
