...
#define TIME(expr) do {							\
    ...
    double time_spent = static_cast<double>(ts_end.tv_sec - ts_begin.tv_sec) + static_cast<double>(ts_end.tv_nsec - ts_begin.tv_nsec)*1e-9; \
...
#define TIMER_STOP(name) do {						\
    ...
    std::fprintf(stderr, "%20.20s:%5d-%-5d %-40.40s %.5e sec \n", __FUNCTION__, ln_begin_##name, ln_end_##name, #name, time_spent); \
  } while(0)
